...
class WpEventMarathon {
    ...
    function rank_users_of_itineraire($itineraire){
		...
		foreach ($users_models as $key => $value) {
	    	...
	    	$users[] = array(
               			...
               			'age' => $age,			   			
               			"dossard" => $value['dossard'],//$dossard_min + $value['dossard'],
               			'surname'=> $value['surname'],
               			'datenaissance'=> $value['datenaissance']
               		);
	    	$users_models_rank[] = $value['time'];
		}
		array_multisort($users_models_rank, SORT_ASC, $users_models);
               	...
               	while ( $query->have_posts() ) : $query->the_post(); 
               		...
               		$courses[] = array(
			    		...
			    		"participants" => array(),
			   			...
			   			"fin" => get_post_meta($post_id, 'date_end_',true),
			   			"debut_hr" => get_post_meta($post_id, 'time_start',true),
			   			"fin_hr" => get_post_meta($post_id, 'time_end_',true),
    ...
    function ajax_callback(){
		...
		if($module == 'get_itineraire'){
	    	...
	    	if($user_id && $course ){
			    ...
			    update_post_meta($post_id, 'date_end_', $itineraire['fin']);
			    update_post_meta($post_id, 'time_end_', $itineraire['fin_hr']);
	    	...
	    	if($user_id && $course ){
			    ...
			    update_post_meta($post_id, 'date_end_', date("Y-m-d"));
			    update_post_meta($post_id, 'time_end_', date("h:i"));
	...
	**/
	function wporg_add_custom_box()
	{
	    ...
	    foreach ($screens as $screen) {
	        add_meta_box(
		        ...
		        'A propos de la Compétition',  //Box title
	...
	function wporg_add_custom_box_itineraire()
	{
	    ...
	    foreach ($screens as $screen) {
	        add_meta_box(
		        'post_event_informations',//Unique ID
		        'A propos da la Compétition',  //Box title
    ...
    **/
    ...
    function custom_page_template($single) {
		...
		}else if($post->post_name == $this->pages['toutes-courses']['slug']){
			if ( file_exists( WPEVENTMARATHON_DIR.'template/html/toutes-courses.php' ) ) {
				$single =  WPEVENTMARATHON_DIR.'template/html/toutes-courses.php' ;
		    }
		}
		return $single;
	}
