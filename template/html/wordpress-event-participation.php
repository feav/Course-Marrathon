    ...
    **/
    ...
    function ajax_callback(){
		...
		if($module == 'delete_email'){
		    	...
		    	if(!is_null($participant)){
		    		...
		    		$body	= '
				    			<div style="padding: 15px 30px 30px 30px;text-align: left;font-family: Helvetica, Arial, sans-serif,Open Sans;line-height: 25px;">
				    				Hi,<br><br>
				    				Fellicitation vous avez termine la course  ' . get_the_title( $event ). '
				    				<br>
				    				Votre chrono est de '. $time .'.
				    				<br>
				    				Pour avoir acces aux resultats, veuillez vous rendre a ce lein.
				    				<a href="' . $link  . '">Les resultats</a>.<br><br>
									   	Cordially,<br>
								</div>
							';

					wp_mail($participants[$key]['email'] , 'Confirmation de score ' . get_the_title( $event ) . ' !', $body, $this->headers );
		    		...
		    		if( $find  === 0 || $find > 0 ){
		    			foreach ($list_users as $indice => $value) {
		    				if($email == $value['email']){
								...
								//wp_mail(  get_option('admin_email', true) ,'Mise a jour des informations de la course !', $mail, $this->headers );
			...
			// 	$mail = str_replace($value['key'], $value['value'], $mail);
			// }
			// // wp_mail( 'feavfeav@gmail.com' , 'Inscription a la course !', $mail );
			// wp_mail( 'feavfeav@gmail.com' , 'Inscription a la course !', 'bonjour' );
			// var_dump($variables);
			// die();
	...
	**/
	function wporg_add_custom_box()
	{
	    ...
	    foreach ($screens as $screen) {
	        add_meta_box(
		        'Events_',//Unique ID
		        'Details Des Participaton',  //Box title
